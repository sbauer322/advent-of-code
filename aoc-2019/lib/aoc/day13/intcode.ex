defmodule AOC.Day13.Intcode do
  @moduledoc false
  ...
  def read(memory, address) when is_integer(address) do
  ...
  def update_instruction_pointer(memory, value) when is_integer(value) do
    Map.put(memory, :pointer, value)
  end

  @spec increment_instruction_pointer(memory, integer) :: memory
  def increment_instruction_pointer(memory, value) when is_integer(value) do
  ...
  def insert_input_at(memory, index, value) when is_integer(value) do
  ...
  def append_input(memory, value) when is_integer(value) do
    inputs = memory.inputs
  ...
  def push_output(memory, value) when is_integer(value) do
    outputs = memory.outputs
    Map.put(memory, :outputs, [value | outputs])
  end

  @spec empty_outputs(memory) :: {list(integer), memory}
  def empty_outputs(memory) do
    outputs =
      read_outputs(memory)
      |> Enum.reverse()

    memory = Map.put(memory, :outputs, [])
    {outputs, memory}
  end

  @spec update(memory, {integer, integer}, integer) :: memory
  def update(memory, {address, mode}, value) when is_integer(value) do
    case mode do
