defmodule AOC.Day9.IntcodeTest do
  alias AOC.Day9.Intcode
  ...
  test "relative base" do
    puzzle_input = ~w(109 19 204 -34)
    user_input = []

    {_state, memory} =
      Intcode.puzzle_input_to_map(puzzle_input, user_input)
      |> (&Map.put(&1, :relative_base, 2000)).()
      |> Intcode.compute()

    assert [0] == Intcode.read_outputs(memory)
  end

  test "no input but copies itself as output (quine)" do
    puzzle_input = ~w(109 1 204 -1 1001 100 1 100 1008 100 16 101 1006 101 0 99)
    user_input = []
    ...
    assert expected == Enum.reverse(Intcode.read_outputs(memory))
  end

  test "outputs a 16 digit number" do
    puzzle_input = ~w(1102 34915192 34915192 7 4 7 99 0)
    user_input = []
    ...
    assert [1_219_070_632_396_864] == Intcode.read_outputs(memory)
  end

  test "outputs the large number in the middle of the input" do
    puzzle_input = ~w(104 1125899906842624 99)
    user_input = []
    ...
    assert [1_125_899_906_842_624] == Intcode.read_outputs(memory)
  end

  test "part 1" do
    {_state, memory} = Intcode.part1("./resources/day9_part1_input.txt", [1])
    assert [2_682_107_844] == Intcode.read_outputs(memory)
  end

  test "part 2" do
    {_state, memory} = Intcode.part2("./resources/day9_part1_input.txt", [2])
    assert [34_738] == Intcode.read_outputs(memory)
  end
end
